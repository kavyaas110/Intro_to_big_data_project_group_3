...
query = """
SELECT t.tconst, p.primaryname AS directorname
FROM Title t
JOIN Directs d ON d.tconst = t.tconst
...
# 3. Group directors per title
grouped = df.groupby('tconst')['directorname'].apply(lambda x: list(set(x)))
transactions = grouped.tolist()
print("Step 3 done: Transactions =", len(transactions))
# 4. Filter infrequent directors
...
filtered_transactions = [
    [director for director in tx if director in frequent_directors]
    for tx in transactions
    if sum(director in frequent_directors for director in tx) >= 2  # skip solo-director
]
print("Step 4 done: Filtered transactions =", len(filtered_transactions))

# 5. Encode transactions with sparse matrix
...
# 7. Output: Co-director rules
print("\nTop Co-Director Rules:")
